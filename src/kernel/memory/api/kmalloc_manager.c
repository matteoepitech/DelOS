/*
** DELOS PROJECT, 2025
** src/kernel/memory/api/kmalloc_manager
** File description:
** kmalloc_manager source file
*/
...
kmalloc_header_t *kernel_heap_lh = NULL; // The last created header for the linked list (lh = last header)
...
kmalloc_init(void)
{
    ...
    if (first_frame == NULL) {
        KPANIC("Failure for malloc initialization due to no more physical space available.");
        return KO_FALSE;
    }
